...
export default function EventFormPage() {
  ...
  const addAvailability = () => {
    setForm({
      ...form,
      availability: [...form.availability, { dayOfWeek: "", startTime: "", endTime: "", duration: "" }]
    })
  }
  ...
  return (
    <div className="max-w-2xl mx-auto mt-10 bg-white rounded-lg shadow p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {form.availability.map((slot, idx) => (
            <div key={idx} className="flex gap-2 items-end mb-2">
             ...
             <Input
                ...
                value={slot.duration}
                onChange={(e) => handleAvailabilityChange(idx, "duration", e.target.value)}
                required
                min={1}
              />
              <Button type="button" variant="destructive" onClick={() => removeAvailability(idx)}>
                Remove
              </Button>
            </div>
          ))}
        ...
        {success && <div className="text-green-600 text-sm">Event created! Redirecting...</div>}
